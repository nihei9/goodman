...
static good_AST *good_rewrite_rhs_elem_ast(good_AST *root_ast, grm_SymbolID target_id, grm_SymbolID replacement_id);
static good_AST *good_new_prule_ast(grm_SymbolID lhs_id, grm_SymbolID rhs_elem_id);
    ...
    for (prule_ast = good_get_child(root_ast, PRULE_OFFSET); prule_ast != NULL; prule_ast = prule_ast->brother) {
        ...
        for (rhs_ast = good_get_child(prule_ast, RHS_OFFSET); rhs_ast != NULL; rhs_ast = rhs_ast->brother) {
            ...
            for (rhs_elem_ast = good_get_child(rhs_ast, RHS_ELEM_OFFSET); rhs_elem_ast != NULL; rhs_elem_ast = rhs_elem_ast->brother) {
                ...
                // 終端記号として名前を登録
                new_lhs_id = grm_put_in_symtbl(symtbl, new_lhs_str, grm_SYMTYPE_TERMINAL);
                if (new_lhs_id == NULL) {
                    return NULL;
                }
